...
#def get_exc_prob_test(rho_sol, t, N):
#    expect_value_P_i = np.zeros([N,len(t)])
#    for i in range(N, 2*N):
#        base_state_vector = np.zeros(2*N)
#        base_state_vector[i] = 1
#        projection_to_u_i = np.mat(base_state_vector).T @ np.mat(base_state_vector) 
#        for j in range(len(t)):
#            expect_value_P_i[i-N,j] = np.trace( projection_to_u_i @ rho_sol[:,:,j] )
#    
#    # now I must summ through i indices
#    return expect_value_P_i.sum(axis=0)